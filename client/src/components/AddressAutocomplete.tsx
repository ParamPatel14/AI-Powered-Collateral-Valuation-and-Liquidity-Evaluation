    ...
    <div className="relative">
      ...
      {(open || loading || error) && (
        <motion.div
          ...
          className="absolute z-[2000] mt-2 w-full overflow-hidden rounded-2xl border border-white/12 bg-[rgba(var(--glass),0.96)] shadow-[0_22px_60px_-34px_rgba(15,23,42,0.22)] backdrop-blur-[18px]"
        >
          {loading && (
            ...
            <div className="max-h-72 overflow-auto">
              {suggestions.map((s) => (
                <button
                  ...
                  className={cn(
                    'flex w-full items-start border-b border-white/10 bg-[rgba(var(--glass),0.9)] px-4 py-3 text-left text-sm font-medium leading-snug text-white/90 transition hover:bg-[rgba(var(--brand),0.12)]',
                  )}
                >
                  {s.description}
                </button>
